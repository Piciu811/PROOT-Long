...
static void present(){
  for(int y=0;y<180;y++) for(int x=0;x<640;x++) nativeFrame[(size_t)x*180+(179-y)]=screen[(size_t)y*640+x];
...
static const uint8_t DIG[12][5]={
 ...
 {0x36,0x49,0x49,0x49,0x36},{0x06,0x49,0x49,0x29,0x1E},{0,0x36,0x36,0,0},{0,0x60,0x60,0,0}};
static void glyph(int x,int y,int id,int s,uint16_t col){ for(int cx=0;cx<5;cx++) for(int cy=0;cy<7;cy++) if(DIG[id][cx]&(1<<cy)) rect(x+cx*s,y+cy*s,s,s,col); }
static void num(int x,int y,const char*t,int s,uint16_t col){
  while(*t){ int id=-1; if(*t>='0'&&*t<='9') id=*t-'0'; else if(*t==':') id=10; else if(*t=='.') id=11;
    if(id>=0) glyph(x,y,id,s,col); else if(*t=='+'||*t=='-'){ rect(x,y+3*s,5*s,s,col); if(*t=='+') rect(x+2*s,y+s,s,5*s,col); }
    x+=6*s; t++;
  }
}
static void numTallBold(int x,int y,const char*t,int sx,int sy,uint16_t col){
  while(*t){ int id=-1; if(*t>='0'&&*t<='9')id=*t-'0'; else if(*t==':')id=10; else if(*t=='.')id=11;
    if(id>=0) for(int cx=0;cx<5;cx++) for(int cy=0;cy<7;cy++) if(DIG[id][cx]&(1<<cy)) rect(x+cx*sx,y+cy*sy,sx+1,sy,col);
    x+=6*sx; t++;
  }
}
static void numFullScreenBold(int x,int y,const char*t,int sx,int sy,uint16_t col){
  while(*t){ int id=-1; if(*t>='0'&&*t<='9')id=*t-'0'; else if(*t==':')id=10; else if(*t=='.')id=11;
    if(id>=0) for(int cx=0;cx<5;cx++) for(int cy=0;cy<7;cy++) if(DIG[id][cx]&(1<<cy)) rect(x+cx*sx,y+cy*sy,sx+3,sy+1,col);
    x+=6*sx; t++;
  }
}
...
static void text5(int x,int y,const char*t,int s,uint16_t col){ while(*t){ char ch=*t++; if(ch==' '){x+=4*s;continue;} int id=-1; if(ch>='A'&&ch<='Z')id=ch-'A'; else if(ch>='a'&&ch<='z')id=ch-'a'; if(id>=0) for(int cx=0;cx<5;cx++) for(int cy=0;cy<7;cy++) if(ALPHA[id][cx]&(1<<cy)) rect(x+cx*s,y+cy*s,s,s,col); x+=6*s; } }
static void scanRaceBoxes(){
  NimBLEDevice::init(""); NimBLEDevice::setPower(ESP_PWR_LVL_P9); NimBLEScan *scan=NimBLEDevice::getScan(); scan->setActiveScan(true); scan->setInterval(100); scan->setWindow(99);
  NimBLEScanResults found=scan->getResults(10000,false); raceboxCount=0; bleSeen=found.getCount(); NimBLEUUID rbService("6E400001-B5A3-F393-E0A9-E50E24DCCA9E");
  int used[8]; for(int i=0;i<8;i++) used[i]=-1;
  for(int slot=0;slot<8;slot++){ int best=-1,bestRssi=-999; for(int i=0;i<found.getCount();i++){ bool already=false; for(int k=0;k<slot;k++) if(used[k]==i) already=true; if(already) continue; const NimBLEAdvertisedDevice *d=found.getDevice(i); if(d&&d->getRSSI()>bestRssi){best=i;bestRssi=d->getRSSI();} }
    if(best<0) break; used[slot]=best; const NimBLEAdvertisedDevice *d=found.getDevice(best); String name=d->haveName()?String(d->getName().c_str()):String("BLE"); bool serviceMatch=d->isAdvertisingService(rbService);
    raceboxes[raceboxCount]=name; raceboxAddr[raceboxCount]=String(d->getAddress().toString().c_str()); raceboxAddrType[raceboxCount]=d->getAddress().getType();
    Serial.printf("BLE candidate %d name='%s' addr=%s type=%u RSSI=%d RBsvc=%d\n",raceboxCount+1,name.c_str(),raceboxAddr[raceboxCount].c_str(),raceboxAddrType[raceboxCount],d->getRSSI(),serviceMatch); raceboxCount++;
  }
}
static void saveRaceBox(const String &addr){ prefs.begin("proot",false); prefs.putString("rbAddr",addr); prefs.end(); savedRbAddr=addr; }
...
static uint8_t rbStream[512]; static size_t rbStreamLen=0;
...
static void drawRaceBoxLive(){
  ...
  char lapNo[3];snprintf(lapNo,sizeof(lapNo),"%02u",(unsigned)(lapCount%100u));text5(243,156,"LAP",2,yellow);numTallBold(288,122,lapNo,7,7,yellow);uint16_t darkgray=C(0x2104);rect(287,0,80,80,darkgray);text5(295,28,"STOP",3,white);if(touchLocked)text5(303,88,"RAIN",2,white);
  if(!timingStopped){char lt[16],bt[16],dt[16],ang[12];fmtLap(lapLastMs,lt,sizeof(lt));fmtLap(lapBestMs,bt,sizeof(bt));if(lapDeltaValid)fmtDelta(lapDeltaMs,dt,sizeof(dt));else snprintf(dt,sizeof(dt),"+0.000");num(382,0,dt,7,lapDeltaValid?(lapDeltaMs<=0?green:red):white);text5(382,74,"LEAN",2,yellow);snprintf(ang,sizeof(ang),"%+.1f",lean);num(442,60,ang,6,lightgray);text5(382,119,"L",3,white);num(422,113,lt,4,white);text5(382,153,"B",3,green);num(422,147,bt,4,green);
  }else{int first=(int)lapHistoryPage*3;for(int row=0;row<3;row++){int idx=first+row,y=8+row*55;if(idx>=lapHistoryN)break;char no[4],tm[16];snprintf(no,sizeof(no),"%02d",idx+1);fmtLap(lapHistory[idx],tm,sizeof(tm));uint16_t lc=(lapHistory[idx]&&lapHistory[idx]==lapBestMs)?green:white;num(382,y,no,3,yellow);num(424,y,tm,3,lc);}if(lapHistoryN>3){rect(590,0,50,60,darkgray);rect(590,120,50,60,darkgray);text5(602,8,"UP",1,white);text5(602,158,"DN",1,white);}}
  rect(12,115,70,60,darkgray);uint16_t smCol=(lapClockRunning&&customLineValid)?green:red;text5(23,156,"S",2,smCol);for(int i=0;i<10;i++)rect(35+i,165-i,2,2,smCol);text5(47,156,"M",2,smCol);rect(88,115,70,60,darkgray);text5(103,156,"SAT",2,sats>0?green:red);rect(164,115,70,60,darkgray);text5(179,156,"REC",2,rbRecordingOn?green:red);present();
}
...
void loop(){
  ...
  if(transfer_num<=1&&lcd_PushColors_len>0)lcd_PushColors(0,0,0,0,NULL);processRaceBoxStream();updateLapClock();if(!askLastDevice&&connState!=drawnConnState){drawnConnState=connState;if(connState==CONN_OK&&connIndex>=0){selectedRacebox=connIndex;saveRaceBox(raceboxAddr[selectedRacebox]);}if(connState==CONN_FAIL){while(transfer_num>1){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxList();}}
  static bool timingShown=false;static uint32_t connOkSince=0;if(connState==CONN_OK&&rbConnected){if(!connOkSince)connOkSince=millis();if(!timingShown&&millis()-connOkSince>=750){while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();timingShown=true;}}else{connOkSince=0;timingShown=false;}static uint32_t lastDataDraw=0;if(connState==CONN_OK&&rbConnected&&timingShown&&rbLiveValid&&millis()-lastDataDraw>=100){lastDataDraw=millis();while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}
  int x,y;bool down=readTouch(x,y);if(askLastDevice&&down&&!touchDown){if(y>=108&&y<162&&x>=70&&x<290){askLastDevice=false;selectedRacebox=savedRaceboxIndex;rbConnected=false;startRaceBoxConnect(savedRaceboxIndex);drawnConnState=CONN_WORKING;}else if(y>=108&&y<162&&x>=350&&x<570){askLastDevice=false;while(transfer_num>1){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxList();}}
  else if(connState==CONN_OK){static uint32_t stopPressStarted=0;static bool stopLongDone=false;bool onStop=(x>=287&&x<367&&y>=0&&y<80);if(!touchLocked&&down&&!touchDown&&onStop&&!stopLongDone)stopPressStarted=millis();if(!touchLocked&&down&&stopPressStarted&&!stopLongDone&&millis()-stopPressStarted>=3000u){rbRequestRecording(false);measurementActive=false;measurementLocked=false;lapClockRunning=false;timingStopped=false;lapCount=0;lapLastMs=lapBestMs=0;lapDeltaValid=false;lapHistoryN=0;lapHistoryPage=0;refTraceN=curTraceN=refCursor=0;lastTraceTow=0;lastCrossTow=0;customLineValid=false;customLineArmed=false;customDirectionPending=false;havePrevFix=false;customLat=customLon=customDirX=customDirY=prevLat=prevLon=0;customSavedAt=0;lapFlashStarted=0;prefs.begin("proot",false);prefs.putBool("sfOk",false);prefs.remove("sfLat");prefs.remove("sfLon");prefs.remove("sfDx");prefs.remove("sfDy");prefs.end();stopLongDone=true;stopPressStarted=0;while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}if(!touchLocked&&!down&&touchDown){if(stopPressStarted&&!stopLongDone&&millis()-stopPressStarted<3000u){rbRequestRecording(false);measurementActive=false;measurementLocked=true;timingStopped=true;lapClockRunning=false;lapDeltaValid=false;lapFlashStarted=0;while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}stopPressStarted=0;stopLongDone=false;}if(!touchLocked&&down&&!touchDown&&!onStop){if(x>=12&&x<82&&y>=115&&y<175){saveCustomLine();while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}else if(x>=164&&x<234&&y>=115&&y<175){rbRequestRecording(!rbRecordingOn);while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}else if(x>=590&&y<60&&lapHistoryPage>0){lapHistoryPage--;while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}else if(x>=590&&y>=120&&lapHistoryN>(lapHistoryPage+1)*3){lapHistoryPage++;while(lcd_PushColors_len>0){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxLive();}}}
  else if(connState!=CONN_OK&&down&&!touchDown&&x>=390&&x<580&&y>=12&&y<54){int pages=max(1,(raceboxCount+2)/3);listPage=(listPage+pages-1)%pages;while(transfer_num>1){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxList();}
  else if(connState!=CONN_OK&&down&&!touchDown&&x>=390&&x<580&&y>=126&&y<168){int pages=max(1,(raceboxCount+2)/3);listPage=(listPage+1)%pages;while(transfer_num>1){lcd_PushColors(0,0,0,0,NULL);delay(1);}drawRaceBoxList();}
  else if(connState!=CONN_OK&&down&&!touchDown&&x>=12&&x<312&&y>=18&&y<156){int row=(y-18)/50,idx=listPage*3+row;if(idx>=0&&idx<raceboxCount){selectedRacebox=idx;rbConnected=false;startRaceBoxConnect(idx);}}
