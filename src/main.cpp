...
static void updateLapClock(){
  ...
  if(customLineValid&&!customDirectionPending&&havePrevFix&&!timingStopped){double x0=localX(customLon,customLat),y0=localY(customLat),px=localX(prevLon,customLat)-x0,py=localY(prevLat)-y0,cx=localX(lon,customLat)-x0,cy=localY(lat)-y0,prevAlong=px*customDirX+py*customDirY,curAlong=cx*customDirX+cy*customDirY,lateral=fabs(cx*(-customDirY)+cy*customDirX);if(curAlong<-5.0)customLineArmed=true;
...
static void drawRaceBoxLive(){
  uint16_t black=C(0x0000),white=C(0xFFFF),green=C(0x07E0),gray=C(0x4208),red=C(0xF800),yellow=C(0xFFE0);float speed,lean;uint8_t fix,sats;uint32_t packets,tow;bool valid;
  ...
  if(!timingStopped){char lt[16],bt[16],dt[16],ang[12];fmtLap(lapLastMs,lt,sizeof(lt));fmtLap(lapBestMs,bt,sizeof(bt));if(lapDeltaValid)fmtDelta(lapDeltaMs,dt,sizeof(dt));else snprintf(dt,sizeof(dt),"+0.000");num(382,0,dt,7,lapDeltaValid?(lapDeltaMs<=0?green:red):white);text5(382,58,"LEAN",2,yellow);snprintf(ang,sizeof(ang),"%+.1f",lean);num(442,55,ang,4,white);text5(382,119,"L",3,white);num(422,113,lt,4,white);text5(382,153,"B",3,green);num(422,147,bt,4,green);
